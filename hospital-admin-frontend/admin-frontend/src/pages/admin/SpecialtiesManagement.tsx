...
export default function SpecialtiesManagement() {
  ...
  const handleSearch = () => {
    ...
    const filtered = localSpecialties.filter((specialty) =>
      specialty.nombre.toLowerCase().includes(searchName.toLowerCase())
    );
  ...
  const handleEdit = (specialty: Specialty) => {
    ...
    setFormData({ nombre: specialty.nombre });
  ...
  const confirmDelete = () => {
    ...
    const updatedSpecialties = specialties.filter((spec) => spec.id !== specialtyToDelete);
    setSpecialties(updatedSpecialties);
